...
int OpenSerial(char *device_name)
{
    ...
    newtio.c_lflag 	= 0;
    newtio.c_cc[VTIME] 	= 0;
    newtio.c_cc[VMIN] 	= (sizeof(struct_ROS_RX_STM_Data));
...
{
    int fd = *((int*)&fdt);
    unsigned char RXRawData[sizeof(struct_ROS_RX_STM_Data)];
        ...
        while(ContinueWhile)
        {
            ...
            case 1:
                if(read((int)fd, &RXRawData[0], 1) == 1)
                {
                    if(RXRawData[0] == 0x43)
                    {
			            StrROS_RX_Angle_Data.Header[0] = 0x43;
                        ParsingMode = 2;
                    }
                    else
                        ParsingMode = 1;
            ...
            case 2:
                if(read((int)fd, &RXRawData[1], 1) == 1)
                {
                    if(RXRawData[1] == 0x21)
                    {
			            StrROS_RX_Angle_Data.Header[1] = 0x21;
                        ParsingMode = 3;
                    }
                    else
                        ParsingMode = 1;
            ...
            case 4:
                ...
                {
                    Water_detected_value = int(RXRawData[3]);
                    // printf("[Water_detect_value] \t %d\n", Water_detected_value);
		            StrROS_RX_Angle_Data.Water_detected = Water_detected_value;
                }
                ParsingMode = 1;
                break;
