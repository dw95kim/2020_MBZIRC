...
class detector:
    def __init__(self):
        ...
        self.detect_pub = rospy.Publisher("/uav1/detection", Float32MultiArray, queue_size=1)
        self.bridge = CvBridge()
        #self.image_sub = rospy.Subscriber("/scout/forward/image_raw", Image, self.image_cb, queue_size=1, buff_size=2**24)
        self.image_sub = rospy.Subscriber("/CAM/image_raw", Image, self.image_cb, queue_size=1, buff_size=2**24)
    ...
    def image_cb(self, data):
        ...
        for i in range(len(objects)):
            ...
            if ((objects[i][0] == 2) or (objects[i][0] == 2)):
                self.flag = 1
